...
\begin{enumerate}
    \item 变量和常量的定义
    \begin{itemize}
        \item 变量名 数据类型 \lstinline{[[NOT NULL] := expr]}
        \item 变量名 数据类型 \lstinline{[[NOT NULL] expr]}
    \end{itemize}
    \item 常量定义
    \begin{itemize}
        \item 常量名 数据类型 \lstinline{CONSTANT := expr}
        \item 常量必须要赋予一个值，并且该值在存在期间或常量的作用域内不能改变。如果试图修改它，过程化SQL将返回一个异常
    \end{itemize}
    \item 赋值语句
    \begin{itemize}
    \item \lstinline{var_name := expr}
    \end{itemize}
\end{enumerate}
\end{frame}
...
\begin{block}{}
\begin{lstlisting}[xleftmargin=0.1\textwidth,linewidth=\linewidth]
DECLARE
...
\end{block} 
    ...
    \item 参数模式：IN（输入）、OUT（输出）、INOUT（输入输出），默认是 IN
    \item 参数调用：IN 需传值，OUT 通过返回值带出，INOUT 传入初值并返回最终值
    \item 参数定义：参数模式 + 参数名 + 数据类型
...
\begin{itemize}
    ...
    \item 求解思路
    \begin{itemize}
        \item 课程号“81001”成绩 85 分，对应区间 [80,89]，学分绩点为 3
        \item 类似地，“81002”对应学分绩点为 4，“81003”对应学分绩点为 3
        \item 三门课学分均为 4，平均学分绩点为
        \item $\mathrm{GPA}=\frac{\sum(\text{学分}\times\text{绩点})}{\sum \text{学分}}=\frac{3\times 4 + 4\times 4 + 3\times 4}{12}=3.33$
    \end{itemize}
       \begin{figure}
        \centering
        \includegraphics[width=0.8\textwidth]{figure/fig-9.jpg}
...
\framebreak
\begin{block}{}
\begin{lstlisting}[firstnumber=15,linewidth=\linewidth]
    BEGIN 
        totalGPA := 0;
        totalCredit := 0;
